package com.bucket.list.completList.contorller;


import com.bucket.list.completList.dto.CompletedListRequestDto;
import com.bucket.list.completList.entity.CompletedList;
import com.bucket.list.completList.mapper.CompletedListMapper;
import com.bucket.list.completList.service.CompletedListService;
...
public class CompletedListController {
    ...
    public ResponseEntity createCompletedList(@Positive @PathVariable("group-id") long groupId,
                                              ...
                                              @RequestPart("data") CompletedListRequestDto.CreateCompletedListDto createCompletedListDto,
        ...
        return new ResponseEntity<>(new SingleResponseWithMessageDto<>(mapper.completeListToCompletedInfo(completedList),"SUCCESS"),HttpStatus.CREATED);
    }
    ...
    public ResponseEntity getCompletedList(@Positive @PathVariable("group-id") long groupId,
        ...
        return new ResponseEntity<>(new SingleResponseWithMessageDto<>(mapper.completeListToCompletedInfo(completedList),"SUCCESS"),HttpStatus.OK);
    }
    ...
    public ResponseEntity updateCompletedList(@Positive @PathVariable("group-id") long groupId,
                                              @Positive @PathVariable("bucket-list-id") long bucketListId,
                                              @Positive @PathVariable("completed-list-id")long completedListId,
                                              @Validated @RequestPart("data") CompletedListRequestDto.UpdateCompletedListDto updateCompletedListDto,
                                              @RequestPart(name = "file", required = false) List<MultipartFile> files){
        ...
        CompletedList completedList = completedListService.updateCompletedList(mapper.updateCompletedListToCompletedList(updateCompletedListDto),files);
        return new ResponseEntity<>(new SingleResponseWithMessageDto<>(mapper.completeListToCompletedInfo(completedList), "SUCCESS"),HttpStatus.OK);
    }
}
